...
![image-20201026232542786](/home/wucp/git/json/img/image-20201026232542786.png)
...
因为作业没有要求我们实现Unmarshal部分，但是为了方便比较和参考，所以使用案例中用了encoding/json的Unmarshal来把经过序列化之后的字节切片又转化成结构体并打印，并且对两个包序列化的字节切片做比较。
...
以`func JsonMarshal(v interface{}) ([]byte, error)`为例，说明本包如何进行单元测试。
...
参考encode.go的Marshal函数，通读源码，并进行简单的使用，了解json在golang中的使用和存储，对golang的duck typing模式做简单的复习与总结。老师说这次作业算是把别人的半个包“搬”成自己的，“以后会经常这样做”，本来不解其意，这次算是学到了吧。
